...
public class PlayerManager : MonoBehaviour
{
    ...
    {
        // Pickup food from the shelf by detecting tagged food items
        if (other.CompareTag("food") && !isCarryingFood)
        {
            Debug.Log("Food detected: " + other.name);  // Debug message
        ...
        if (other.CompareTag("coffeeplace") && isCarryingFood)
        {
            ...
            // Check if the food was placed in the customer's foodplace, then remove the chatbox
            var customerChat = other.transform.parent.GetComponent<CustomerChat>();

            // Debugging to check if the customer and chatbox were found
            if (customerChat != null)
            {
                Debug.Log("Customer chatbox found. Removing chatbox...");
                customerChat.RemoveChatbox();  // Remove the chatbox after placing the food
            }
            else
            {
    ...
    {
        if (other.CompareTag("food") || other.CompareTag("coffeeplace"))
        {
